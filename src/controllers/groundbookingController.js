    ...
    try {
        ...
        const existingBooking = await pool.query(
            `SELECT booking_id
             ...
             WHERE ground_name = $1
               AND booking_date = $2
               AND time_slot = $3`,
            [ground_name, booking_date, time_slot]
        );
        ...
        if (existingBooking.rows.length > 0) {
            return res.status(409).json({
                success: false,
                statusCode: 409,
                message: "This ground is already booked for the selected date and time slot."
            });
        }
        ...
        return res.status(201).json({
            success: true,
            statusCode: 201,
            message: "Ground booking created successfully.",
            data: result.rows[0]
        });
...
exports.updateGroundBooking = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return sendErrorResponse(res, 404, "Ground booking not found");
    }
